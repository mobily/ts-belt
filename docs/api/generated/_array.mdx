...
```ts
function unzip<A, B>(xs: ReadonlyArray<[A, B]>): [ReadonlyArray<A>, ReadonlyArray<B>]
```
...
```ts
function zip<A, B>(xs0: ReadonlyArray<A>, xs1: ReadonlyArray<B>): ReadonlyArray<[A, B]>
function zip<A, B>(xs1: ReadonlyArray<B>): (xs0: ReadonlyArray<A>) => ReadonlyArray<[A, B]>
```
