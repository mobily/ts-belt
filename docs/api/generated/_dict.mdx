...
```ts
function deleteKeys<T, K extends keyof T>(dict: T, keys: K[]): Omit<T, K>
function deleteKeys<T, K extends keyof T>(keys: K[]): (dict: T) => Omit<T, K>
```
...
```ts
function fromPairs<T, K extends keyof any>(xs: Array<readonly [K, T]>): Record<K, T>
```
...
```ts
function selectKeys<T, K extends keyof T>(dict: T, keys: K[]): Pick<T, K>
function selectKeys<T, K extends keyof T>(keys: K[]): (dict: T) => Pick<T, K>
```
...
```ts
function toPairs<T, K extends keyof any>(dict: Record<K, T>): Array<readonly [K, T]>
```
