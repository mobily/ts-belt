...
```ts
function update<T, K extends keyof T, R>(dict: T, key: K, fn: (value: T[K]) => R): T & Record<K, R>
function update<T, K extends keyof T, R>(key: K, fn: (value: T[K]) => R): (dict: T) => T & Record<K, R>
```
...
D.update({ 0: false, 1: true }, 1, v => +v) // → { 0: false, 1: 1 }
pipe(
  { name: 'Joe', location: 'Warsaw' },
