...
```ts
D.deleteKeys(joe, []) // → { name: 'Joe', location: 'Warsaw' }
D.deleteKeys(joe, ['name']) // → { location: 'Warsaw' }
D.deleteKeys(joe, ['name', 'location']) // → {}
pipe(joe, D.deleteKeys([])) // → {
  name: 'Joe',
  location: 'Warsaw',
}
pipe(joe, D.deleteKeys(['name'])) // → { location: 'Warsaw' }
pipe(joe, D.deleteKeys(['name', 'location'])) // → {}
```
