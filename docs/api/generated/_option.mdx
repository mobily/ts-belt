...
```ts
function fromPredicate<T>(value: T, predicateFn: (value: NonNullable<T>) => boolean): Option<ExtractValue<T>>
function fromPredicate<T>(predicateFn: (value: NonNullable<T>) => boolean): (value: T) => Option<ExtractValue<T>>
```
...
```ts
function tap<T>(option: Option<T>, someFn: (value: T) => void): Option<T>
