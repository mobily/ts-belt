...
let map = (mapFn, option) => Belt.Option.map(option, mapFn)
let flatMap = (mapFn, option) => Belt.Option.flatMap(option, mapFn)
let mapWithDefault = (defaultValue, mapFn, option) =>
  Belt.Option.mapWithDefault(option, defaultValue, mapFn)
...
let filter = (predicateFn, option) =>
  Belt.Option.flatMap(option, value => predicateFn(value) ? Some(value) : None)
